...
| `const:fatigue-factor-combat` | Specialization fatigue factor — Combat | **1.8** | x level (plus 0.02) | Level term = floor(level x (0.02 + 1.8)): +1 at L1 (measured exact on a clean L1 Combat character), +109 at L60 (unmeasured directly; follows the formula). |
| `const:fatigue-factor-magic` | Specialization fatigue factor — Magic | **0.9** | x level (plus 0.02) | Level term = floor(level x (0.02 + 0.9)) = +55 at L60 — measured exact on two independent clean L60 Magic characters (the value previously recorded as a universal '+55 at L60'). |
| `const:fatigue-factor-stealth` | Specialization fatigue factor — Stealth | **3.0** | x level (plus 0.02) | Level term = floor(level x (0.02 + 3.0)) = +181 at L60 — measured exact on a clean naked zero-mastery L60 Stealth character (581 = 400 + 181), the reading previously misread as a +126 'character-specific bug'. |
...
| `const:native-magicka-regen` | Native magicka regen | **1** | magicka/tick | For all characters EXCEPT Stunted Magicka (Atronach). Deliberate NP GMST edit. An earlier community statement (2025-03-23) put native regen at 5% of max for HP/MP/Fatigue alike; for magicka the 2026-08-21 measured 1/tick supersedes it. |
| `const:enchant-skill-one-charge` | Enchant skill for 1-charge casts | **110** | skill | At 110 Enchant, all enchanted items cost 1 charge per use (multiple corroborations 2019-2022). Maintainer-confirmed still accurate on the current (0.8) server, 2026-08-26 — the pre-0.8 sourcing is historical provenance only, not a caveat. |
| `const:max-effects-per-enchant` | Max effects per enchant/spell | **8** | effects | Soul size raises magnitude/duration, never the effect count. Maintainer-confirmed still accurate on the current (0.8) server, 2026-08-26 — the pre-0.8 sourcing is historical provenance only, not a caveat. |
...
### Max Fatigue  
`maxFat = STR + WIL + AGI + END + floor(level x (0.02 + spec_fatigue_factor)) + flat_fortify_fatigue`  
Spec fatigue factors (fact:spec-fatigue-factors): Combat 1.8 / Magic 0.9 / Stealth 3.0 — L60 level terms 109 / 55 / 181. Decoded 2026-08-28 from the community wiki's formula, which reconciles EXACTLY with all four clean naked zero-mastery readings: L1 Combat 181 = 60+30+40+50 + floor(1.82)=1; L60 Magic 470 = 100+115+100+100 + 55 (a racial Fortify Willpower +15 counts in the sum) and 515 = 120+120+120+100 + 55 (vampirism attribute fortifies counting); L60 Stealth 581 = 100+100+100+100 + floor(181.2)=181. The Stealth reading was previously recorded as an unexplained +126 surplus — it was the Stealth factor all along (181-55=126), not a bug. Attribute fortifies (racial, vampiric, gear) count in the sum; flat Fortify Fatigue effects add after.
...
- **atronach-stunted-regen**: Stunted Magicka (Atronach) removes the native 1 magicka/tick regen AND halves MASTERY-GRANTED HoT/MoT/FoT tick regen ONLY — item-granted regen is NOT halved (maintainer-confirmed 2026-08-26: no evidence of item or any other halving). There is no native HP/fatigue regen for it to affect (see the contested native-regen records).
...
- **native-hp-regen**: There is NO native HP regeneration on NP. HP recovers only through actual sources: Restore Health effects, HoT masteries/items, potions, and spells.
- **clothing-armor-slots**: Clothing and armor never interfere. Clothing that always coexists with armor: pants, belt, shirt, skirt, robe. Armor that never interferes with clothing: greaves, cuirass, pauldrons, helm. The only exclusive choices are the two shared body slots: gloves (clothing) OR gauntlets (armor) in the hands slot, and shoes (clothing) OR boots (armor) in the feet slot. No clothing piece conflicts with another clothing piece.
- **native-fatigue-regen**: There is NO native fatigue regeneration on NP. Fatigue recovers only through actual sources: Restore Fatigue effects, FoT masteries/items, potions, and resting.
- **spec-fatigue-factors**: Specialization fatigue factors (community wiki, 'List of notable differences'): Combat 1.8, Magic 0.9, Stealth 3.0. The per-level fatigue term is level x (0.02 + factor), floored — +109 / +55 / +181 at L60. There are no fatigue-factor masteries analogous to the Health Factor line.
