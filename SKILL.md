...
4b. **Quantitative targets** (resist %, armor rating, HP...): verify with explicit arithmetic over the data — racial + gear + mastery magnitudes for resists (note `const:magic-resistance-cap` = 85: raw totals above 85 are deliberate overcap slack against debuffs — say so when relevant); AR via `formula:sheet-ar` (piece AR x slot weight x skill/100, floored, + flat bonuses) at a STATED armor-skill assumption — and when delivering a FILE, set the state's `skl`/`skm`/`skh` fields to that same assumption so the builder's tiles match your arithmetic. If a target can't be reached from the catalog, say what IS achievable and the shortfall — never fudge the numbers to please.
5. Validate before delivering: every race/sign key exact, weapon index in range, every slot value decodes to a real name|owner pair, every mastery name exact, budget arithmetic correct. Then deliver:
   ...
   - **Chat**: formatted for reading, not a data dump. Header line: **Race · Birthsign · Specialization — theme (budget)**. Then a **Masteries table** (columns: Mastery line | Ranks | Cost) ending in a total row with the arithmetic shown. Then a **Gear table** (columns: Slot | Item | Key stats), weapon as the last row. If the user set quantitative targets, a short **Targets table** (Target | Achieved | How) with the arithmetic. Same data, same validation as the file path — a chat build must be exactly loadable if the user later asks for it as a file. End by offering the file version in one short line that includes the builder link (https://gillyguthrie.github.io/np-character-builder/).
